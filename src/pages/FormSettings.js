    ...
    const fetchData = async () => {
      ...
      try {
        if (searchTerm.length === 4) {
          ...
          // 修正：檢查響應結構並使用正確的資料陣列
          const dataArray = response.data.forms || response.data || [];
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <div className="flex justify-between items-center mb-4 gap-4">
              <div className="relative flex-grow max-w-xs">
                ...
                <input type="text" placeholder="請輸入部門代號4碼" value={searchTerm} onChange={handleSearchChange} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md ..."/>
              </div>
